...
def visualization(ori_img, pred, img_name):
    ori_rgb = cv2.cvtColor(ori_img, cv2.COLOR_BGR2RGB)
    overlay = ori_rgb.copy()
    red_mask = np.zeros_like(ori_rgb)
    red_mask[..., 0] = pred  # R 通道
    overlay = cv2.addWeighted(red_mask, 0.5, overlay, 0.5, 0)
    
    fig, axes = plt.subplots(1, 3, figsize=(18, 6))
    axes[0].imshow(ori_rgb);    axes[0].set_title("Raw");    axes[0].axis("off")
    axes[1].imshow(pred, cmap="gray"); axes[1].set_title("Mask");        axes[1].axis("off")
    axes[2].imshow(overlay);    axes[2].set_title("Overlay");     axes[2].axis("off")
    plt.tight_layout()
    save_path = f'/root/UniLSeg/image/ppt/{img_name}.jpg'
...
def get_parser(img_path, text, save_path):
...
def run_demo_jupyter(img_path, text, img_name, save_path='demo.png'):
    args = get_parser(img_path, text, save_path)
    ...
    ori_img = cv2.imread(image_path)
    if ori_img is None:
        logger.error(f"Failed to load image from {image_path!r}. "
                     "Please check that the file exists and the path is correct.")
        raise FileNotFoundError(f"Could not read image at {image_path}")
    # 现在 ori_img 一定不是 None，才安全转换颜色空间
    img = cv2.cvtColor(ori_img, cv2.COLOR_BGR2RGB)
    ...
    ori_img, pred = run_demo(args, model, img, text_prompt, save_path, ori_img, params)
    visualization(ori_img, pred, img_name)
